...
<div class="layout faculty-sidebar-layout">
    ...
    <?php if($mode === 'manage'): ?>
        ...
        <section class="course-banner">
            ...
            <div class="banner-actions">
                <div class="row">
                    <span class="btn-pill"><?php echo e($course->status ?? 'Draft'); ?></span>
                    <a href="<?php echo e(route('faculty.courses.edit', $course->id)); ?>" class="btn-pill">Edit</a>
                </div>
            ...
            <section>
                ...
                <form class="add-module-card" method="POST" action="<?php echo e(route('faculty.module.store', $course->id)); ?>">
                    ...
                    <div class="add-module-inputs">
                        ...
                        <?php echo $__env->make('components.rich-text-editor', ['name' => 'module_description', 'id' => 'module-description-new', 'value' => old('module_description'), 'placeholder' => 'Describe what this module covers...'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    </div>
    ...
    || (int) session('open_lesson_form') === (int) $module->idx; ?>
                        ...
                        <form class="lesson-form" id="lesson-form-<?php echo e($module->idx); ?>"
                            ...
                            <div class="row2">
                                <input class="input-outline" type="text" name="lesson_title"
                                       ...
                                       value="<?php echo e(old('lesson_title')); ?>" required>
                            </div>
                            ...
                            <?php if($errors->has('lesson_content') && (int) session('open_lesson_form') === (int) $module->idx): ?>
                                <div style="background:#fdecea;border:1px solid #f2b8b5;color:#a93226;padding:10px 14px;border-radius:10px;font-size:13px;font-weight:600;">
                                    <?php echo e($errors->first('lesson_content')); ?>
                            ...
                            <div class="actions">
                                ...
                                <button class="btn-cancel-outline" type="button"
                                        onclick="toggleLessonForm(<?php echo e($module->idx); ?>, false)">Cancel</button>
                            </div>
                        </form>
                        ...
                        <?php $__empty_2 = true; $__currentLoopData = $module->lessons ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lesson): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                            <div class="lesson-row">
                                ...
                                <?php if($lesson->id ?? null): ?>
                                    <button type="button" class="btn-edit-lesson"
                                            onclick="toggleLessonEdit(<?php echo e($lesson->id); ?>)"
                            ...
                            <?php if($lesson->id ?? null): ?>
                                <form class="lesson-form lesson-edit-form" id="lesson-edit-<?php echo e($lesson->id); ?>"
                                      style="display:<?php echo e(session('open_lesson_edit') == $lesson->id ? 'flex' : 'none'); ?>;"
                                      method="POST"
                                      action="<?php echo e(route('faculty.lesson.update', [$course->id, $module->idx, $lesson->id])); ?>">
                                    <?php echo csrf_field(); ?>

                                    <div class="row2">
                                        <input class="input-outline" type="text" name="lesson_title"
                                               placeholder="Lesson title *"
                                               value="<?php echo e(old('lesson_title', $lesson->title)); ?>" required>
                                    </div>
                                    ...
                                    <?php if($errors->has('lesson_content') && (int) session('open_lesson_edit') === (int) $lesson->id): ?>
                                        <div style="background:#fdecea;border:1px solid #f2b8b5;color:#a93226;padding:10px 14px;border-radius:10px;font-size:13px;font-weight:600;">
                                            <?php echo e($errors->first('lesson_content')); ?>
                                    ...
                                    <div class="actions">
                                        <input class="input-outline" type="number" name="duration"
                                               placeholder="Duration ( Min )" min="0"
                                               value="<?php echo e(old('duration', $lesson->duration_raw ?: '')); ?>">
                                        <button class="btn-save-lesson" type="submit">Save Lesson</button>
                                        <button class="btn-cancel-outline" type="button"
                                                onclick="toggleLessonEdit(<?php echo e($lesson->id); ?>)">Cancel</button>
                                    </div>
                                </form>
                            <?php endif; ?>
                        ...
                        <div class="quiz-row" id="quiz-row-<?php echo e($module->idx); ?>">
                            <?php if($module->quiz ?? null): ?>
                                ...
                                <form method="POST"
                                      action="<?php echo e(route('faculty.quiz.destroy', [$course->id, $module->idx])); ?>"
                                      style="display:inline-block;margin-left:8px;"
                                      onsubmit="return confirm('Delete &quot;<?php echo e(addslashes($module->quiz->title)); ?>&quot;? Student attempts for this quiz will be removed too. This cannot be undone.');">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit"
                ...
                <?php $__empty_1 = true; $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="student-card">
                        ...
                        <div class="student-info">
                            ...
                            <div class="sub">
                                <?php if($student->institutional_label ?? null): ?>
                                    <span class="status-pill"><?php echo e($student->institutional_label); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                        <span class="status-pill"><?php echo e($student->status); ?></span>
                        <?php if($student->faculty_verification_pending ?? false): ?>
                            <form method="POST" action="<?php echo e(route('faculty.enrollments.verify', $student->enrollment_id)); ?>" style="display:inline-flex;gap:6px;">
                                <?php echo csrf_field(); ?>
    ...
    <?php elseif($mode === 'quiz'): ?>
        ...
        <a class="back-link" href="<?php echo e(route('faculty.courses.manage', $course->id ?? 1)); ?>">â€¹ Back to Modules</a>
        ...
        <form method="POST" action="<?php echo e(route('faculty.quiz.store', [$course->id, $moduleIdx])); ?>">
            ...
            <div id="questions-area" style="display:<?php echo e($isNewQuiz ? 'none' : 'block'); ?>;">
            <div id="questions">
            ...
            <div class="save-quiz-row" id="save-quiz-row" style="display:<?php echo e($isNewQuiz ? 'none' : 'flex'); ?>;">
...
</html><?php /**PATH C:\Users\PaulV\Documents\MICROCREDENTIALS NEW ADDITIONS\UPSKILL - Microcredential Platform\resources\views/faculty/courses/index.blade.php ENDPATH**/ ?>